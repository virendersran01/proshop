...
class OrderSummaryActivity : AppBaseActivity(), PaymentResultListener {
    ...
    private fun updateAddress() {
        ...
        tvUserAddress.text =shipping.first_name+" "+shipping.last_name+"\n"+shipping.getFullAddress(sap = "\n")
        val billing= getbillingList()
        tvBillingAddress.text = billing.first_name+" "+billing.last_name+"\n"+billing.getFullAddress(sap = "\n")
        if (shippingItems!=null){
            tvMethod.show()
            if (shippingItems!!.id=="free_shipping" || shippingItems!!.cost=="0" || shippingItems!!.cost.isEmpty()){
                ...
                tvMethodName.text = shippingItems!!.methodTitle+": "+shippingItems!!.cost.currencyFormat()
            }
        }
    ...
    private fun createOrderRequest(orderRequest: OrderRequest) {
        ...
        val orderItems: ArrayList<Line_items> = intent.getSerializableExtra(Constants.KeyIntent.PRODUCTDATA) as ArrayList<Line_items>
    ...
    private fun getCheckoutUrl(id: Int) {
        var request = CheckoutUrlRequest()
