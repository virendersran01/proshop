...
fun Activity.openNetworkDialog(onClick: () -> Unit)  {
    try{
        if (noInternetDialog == null) {
            ...
            noInternetDialog?.setContentView(R.layout.layout_network); noInternetDialog?.setCanceledOnTouchOutside(false); noInternetDialog?.setCancelable(false)
...
fun AppBaseActivity.socialLogin(email:String, accessToken: String, firstName: String, lastName: String, loginType: String, photoURL: String, onResult: (Boolean) -> Unit, onError: (String) -> Unit) {
    ...
    getRestApiImpl().socialLogin(requestModel, onApiSuccess = {
        showProgress(false)
        saveLoginResponse(it,true, accessToken, onError)
    }, onApiError = {
        showProgress(false)
        snackBar(it)
    })
